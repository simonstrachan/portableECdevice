...
void runNPV(uint8_t user_gain, uint32_t pulse_period, uint32_t pulse_width,
  ...
  startV = refVolt * TIA_BIAS[abs(startV)] * startV / abs(startV);
  endV = determineLMP91000Bias(endV);
  endV = refVolt * TIA_BIAS[abs(endV)] * endV / abs(endV);

  uint8_t pulses = abs((endV - startV) / (refVolt * 0.02));
  ...
  for (uint8_t i = 0; i < pulses; i++) {
    ...
    if (startV != 0){
      nextV = (refVolt * TIA_BIAS[abs(startV)] * startV / abs(startV)) + i * incrV;
    } else {
      nextV += i * incrV;
    }
    ...
    Serial.print((uint16_t)(refVolt * TIA_BIAS[abs(nextV)] * (nextV / abs(nextV))));
    Serial.print(",");
